...
# SITE : Criar conteúdo para MTM e estatística 


# Lista de Exercícios - Capítulo 10 - Teste de hipótese em R
...
install.packages('nycflights13') # dados sobre voos em 3 aeroportos em NY
library('ggplot2')
library('dplyr')
...
samples = rbind(amostra1,amostra2) #rbind -> faz a ligação por linha
View(samples)
...
# H0(huipótese nula => já existe) e 
# H1(hipótese alternativa => é o que queremos provar) 
# devem ser mutuamente exclusivas.

# H0 = Não há diferença significativa entre os atrasos da DL e UA (diff da média de atrasos = 0).
# H1 = Delta (DL) atrasa mais (diff das médias > 0).


# Cria as amostras 
dl <- sample_n(filter(pop_data, carrier == "DL", arr_delay > 0), 1000) # > 0 pq queremos o atraso!!
...
#Criar sol analísticas, obter um problema, app as ferramentas, criar prod/serviço.
# FIM 
