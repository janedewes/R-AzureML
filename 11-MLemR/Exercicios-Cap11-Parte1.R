...
# OBS: O MODELO DE REGRESSÃO SÓ FAZ SENTIDO, SE EXISTIR CORRELAÇÃO ENTRE OS DADOS !!!
# UM MODELO DE ML APRENDE OS COEFICIENTES!!
...
plot(x, y1, pch = 19,  xlab = 'X', ylab = 'Y') # (scatterplot) -> existe uma correlação positiva!



# Crie um modelo de regressão para as duas variáveis x e y1
modelo <- lm(y1 ~ x) # y1 -> var target; x -> var preditora  
modelo
class(modelo)
...
modelo <- lm(Tempo ~ Idade) # Tempo -> var target; Idade -> var preditora
modelo


# Calcule a reta de regressão
y <- a + b*x # eq de regressão ( que contém as previões)
...
# Criando os dados (dados historicos)
...
modelo <- lm(pesos ~ alturas) # var target -> pesos
...
# Gere a linha de regressão (abline -> só passar os coefs)
abline( -70.4627, 0.8528)



# ------------------------------------
# testar o modelo com novos conjuntos de dados: 

# Faça as previsões de pesos com base na nova lista de alturas
alturas2 = data.frame(c(179, 152, 134, 197, 131, 178, 185, 162, 155, 172)) # para usar a fç predict os dados devem estar em um df
...
# Gerando gráficos com a distribuição dos resíduos (erros do modelo)
par(mfrow = c(2,2))
plot(modelo)
