...
curve(dnorm(x, 3, 0.25), add = TRUE, col="red", lwd=1) # d -> desnidade
...
# Solução
# Aplicamos a fç pnorm da distribuição normal com média 72 e desvio padrao 15.2. Como estamos procurando a 
# porcetagem de alunos com pontuação superior a 84, estamos interessados na cauda superior da distribuição normal.
...
# Por isso, a taxa de processamento é 1/3 de checkouts por minuto . Em seguida, aplicamos a fç "pexp" da 
# distribuição exponencial com taxa = 1/3.
pexp(2, rate=1/3)
...
# Solução 
# A prob de ter 14 ou manos carros atravessando a ponte em um determinado minuto é dada pela fç ppois (poison)
?ppois
ppois(14, lambda = 12) #lower tail


# Assim, a prob de ter 15 ou mais carros cruzando a ponte em um minuto está  na cauda superior da fç de
...
# Solução 
# Como apenas 1 das 5 respostas possíveis está correta, a probabilidade de responder corretamente a uma 
# pergunta aleatória é 1/5 = 0,2. Podemos encontrar encontrar a probabilidade de ter exatamente 4 respostas
# corretas por tentativas aleatórias como segue:
dbinom(4, size = 12, prob = 0.2)


# Para encontrar a probabilidade de ter quatro ou manos respostas corretas por tentativas aleatórias, 
# aplicamos a fç dbinom com x= 0, ...,4.
...
# ou então: 
pbinom(4, size = 12, prob = 0.2) # igual ao de cima!
