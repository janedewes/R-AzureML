...
# Coeficiente de Assimetria (Skewness) (direita/esquerda)
...
# Coeficiente de Assimetria (Skewmess)
...
library(moments) # A f; skewness pertence so pacote moments (do R)
?skewness
SK = skewness(dados)
print(SK)
...
# Outro exemplo
set.seed(1234) # p usar os mesmos dados
x = rnorm(1000) # distribuição normal de mil elementos
hist(x) # só o histo não é suficiente para determinar a assimetria
skewness(x) # calculando o skewness podemos saber.
