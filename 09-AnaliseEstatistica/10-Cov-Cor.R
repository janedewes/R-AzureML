...
# mesma def anterior mas com texto diferente:
...
install.packages("ggpubr") #fornece a fç ggscatter
library("ggpubr")
...
# OBS: existem alguns metodos para calcular o coef de correlação o "pearson" é um deles.
...
# Correlação (para usar a escala de [-1 a 1], essa escala é melhor para compreender a relação entre as variáveis)
?cor
cor(x, y)
# no exemplo a correlação é negativa.




# OBS: A correlação não implica em causalidade!! 
